...
export interface BulkHideModalProps {
  tasks: Task[];
...
export function BulkHideModal({ tasks, hiddenTaskIds, onClose, onApply }: BulkHideModalProps) {
  const { t } = useI18n();

  const availableCounts = useMemo(() => {
    const counts: Record<HideableStatus, number> = { done: 0, pending: 0, cancelled: 0 };
  ...
  const statusRows = useMemo(
    () => HIDEABLE_STATUSES.map((status) => ({ status, label: taskStatusLabel(status, t), count: availableCounts[status] })),
    [availableCounts, t],
  );
      ...
      <div className="w-full max-w-sm rounded-2xl border border-slate-700 bg-slate-900 p-5 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-base font-bold text-white">
            {t({ ko: '숨길 상태 선택', en: 'Select statuses to hide' })}
          </h2>
          ...
          {statusRows.map(({ status, label, count }) => (
            ...
            >
              <span className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={selected[status]} disabled={count <= 0}
        ...
        <div className="mt-4 flex items-center justify-end gap-2">
          ...
          <button onClick={() => onApply(selectedStatuses)} disabled={hideTargetCount <= 0}
            ...
            {t({ ko: '숨김 적용', en: 'Apply hide' })} ({hideTargetCount})
          </button>
        </div>
      </div>
    </div>
