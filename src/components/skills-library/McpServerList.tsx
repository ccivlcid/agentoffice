...
import McpServerEditModal, { type McpEditInitialData } from "./McpServerEditModal";
...
export function inferMcpCategory(entry: McpRegistryEntry): string {
  ...
  if (
    /\b(sql|postgres|mysql|sqlite|mongo|redis|database|db|supabase|prisma|drizzle|dynamo|fauna|firebase)\b/.test(text)
  )
    return "database";
  if (
    /\b(api|http|rest|graphql|webhook|fetch|request|oauth|slack|discord|github|gitlab|jira|notion|stripe|twilio|sendgrid|openai|anthropic|google|aws|azure|cloud)\b/.test(
      text,
    )
  )
    return "api";
  if (
    /\b(git|docker|npm|yarn|pnpm|lint|test|debug|build|ci|cd|deploy|kubernetes|k8s|terraform|puppeteer|playwright|selenium|browser|chrome|screenshot|scrape|crawl)\b/.test(
      text,
    )
  )
    return "dev-tools";
...
export default function McpServerList({ t, agents }: McpServerListProps) {
  ...
  const [addInitialData, setAddInitialData] = useState<McpEditInitialData | null>(null);
    ...
    <div className="space-y-4">
          ...
          <div className="px-5 pb-5 pt-0 border-t border-slate-700/50">
            <div className="text-sm text-slate-300 space-y-3 mt-3">
              ...
              <ul className="list-disc list-inside space-y-1.5 text-slate-400">
                <li>
                  {t({
                    ko: "검색/카테고리로 서버를 필터링한 뒤, 카드의 「동기화」로 설정 파일에 반영할 수 있습니다.",
                    en: 'Filter servers by search or category, then use "Sync" on a card to write to config files.',
                  })}
                </li>
                <li>
                  {t({
                    ko: "프리셋에서 인기 MCP 서버를 추가하거나, 레지스트리에서 검색하여 추가할 수 있습니다.",
                    en: "Add popular MCP servers from presets or search the registry.",
                  })}
                </li>
                <li>
                  {t({
                    ko: "MCP 동기화 현황에서 CLI별로 어떤 서버가 동기화되었는지 확인할 수 있습니다.",
                    en: "Check MCP Sync Status to see which servers are synced per CLI.",
                  })}
                </li>
              </ul>
              <p className="text-slate-500 text-xs mt-2">
                {t({
                  ko: "동기화된 서버는 해당 CLI의 설정 파일에서 에이전트가 사용할 수 있습니다.",
                  en: "Synced servers are available to agents through the CLI config files.",
                })}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              ...
              <div className="text-xs text-slate-500">{t({ ko: "등록된 서버", en: "Registered" })}</div>
            </div>
            <button
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            <input
              ...
              placeholder={t({
                ko: "MCP 서버 검색... (이름, 패키지, 설명)",
                en: "Search MCP servers... (name, package, description)",
              })}
      ...
      <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--th-border)", background: "var(--th-bg-surface)" }}>
        ...
        >
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg" style={{ background: "var(--th-bg-sidebar)", border: "1px solid var(--th-border)" }}>
            <RefreshCw width={18} height={18} style={{ color: "var(--th-text-accent)" }} aria-hidden />
          </div>
          ...
          {syncPanelOpen ? <ChevronDown width={18} height={18} style={{ color: "var(--th-text-muted)" }} aria-hidden /> : <ChevronRight width={18} height={18} style={{ color: "var(--th-text-muted)" }} aria-hidden />}
        </button>
        {syncPanelOpen && (
          <div className="px-4 pb-4 pt-0" style={{ borderTop: "1px solid var(--th-border)" }}>
            <div className="mt-3 max-h-[380px] overflow-y-auto space-y-2">
              {filtered.length > 0 ? (
                ...
                <div className="rounded-lg px-3 py-6 text-center text-xs" style={{ background: "var(--th-bg-sidebar)", border: "1px solid var(--th-border)", color: "var(--th-text-muted)" }}>
                  {t({ ko: "등록된 서버가 없습니다", en: "No registered servers" })}
                </div>
              )}
            </div>
            <div className="flex justify-end pt-3">
              <button
                ...
                style={{ background: "var(--th-text-accent)", color: "white" }}
              >
                {mcp.syncResult ? (
                  <>
                  ...
                  <>
                    <RefreshCw width={16} height={16} className={mcp.syncing ? "animate-spin" : ""} aria-hidden />
      ...
      <div className="rounded-xl border border-slate-700/60 bg-slate-900/50 overflow-hidden">
        ...
        >
          ...
          <div className="text-sm font-semibold text-slate-100">{t({ ko: "MCP 레지스트리", en: "MCP Registry" })}</div>
          {mcp.registryTotal > 0 && <span className="text-xs text-slate-500">({mcp.registryTotal})</span>}
          <div className="text-[11px] text-slate-500 ml-auto mr-2">registry.modelcontextprotocol.io</div>
          {registryOpen ? (
            ...
            {mcp.registryLoading && mcp.registry.length === 0 ? (
              <div className="text-center py-8">
                ...
                <div className="text-xs text-slate-500">
                  {t({ ko: "레지스트리 로딩 중...", en: "Loading registry..." })}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2 max-h-[480px] overflow-y-auto pr-1">
                {filteredRegistry.map((entry) => {
                  ...
                  return (
                    <div
                      key={entry.name}
                      className="relative bg-slate-800/30 border border-slate-700/40 rounded-xl p-3 hover:bg-slate-800/50 hover:border-slate-600/50 transition-all"
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex min-w-0 items-start gap-2">
                          {entry.iconUrl ? (
                            <img
                              src={entry.iconUrl}
                              alt=""
                              className="w-7 h-7 rounded-lg shrink-0 bg-slate-900/60 p-0.5"
                            />
                          ) : (
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {npmPkg && (
                              <div className="mt-0.5 truncate text-[10px] text-slate-500 font-mono">
                                {npmPkg.identifier}
                              </div>
                            )}
                          </div>
                        ...
                        <button
                          onClick={() => {
                            if (!alreadyAdded && npmPkg) {
                              const envObj: Record<string, string> = {};
                              for (const ev of npmPkg.envVars || []) envObj[ev.name] = "";
                              setAddInitialData({
                          ...
                          title={
                            alreadyAdded
                              ? t({ ko: "이미 추가됨", en: "Already added" })
                              : !npmPkg
                                ? t({ ko: "npm 패키지 없음", en: "No npm package" })
                                : t({ ko: "추가", en: "Add" })
                          }
                        >
      ...
      {/* Modals */}
      {(mcp.isCreateOpen || mcp.editingServer || addInitialData) && (
        <McpServerEditModal
          server={mcp.editingServer}
          initialData={addInitialData ?? undefined}
          agents={agents}
          submitting={mcp.submitting}
          t={t}
          onClose={() => {
            mcp.setIsCreateOpen(false);
            mcp.setEditingServer(null);
            setAddInitialData(null);
          }}
          onSave={(data) => {
            if (mcp.editingServer) {
              void mcp.handleUpdate(mcp.editingServer.id, data);
            } else {
              void mcp.handleCreate(data as Parameters<typeof mcp.handleCreate>[0]).then(() => setAddInitialData(null));
            }
          }}
        />
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-80 rounded-2xl border border-slate-700 bg-slate-800 p-6">
            ...
            <p className="text-xs text-slate-400 mb-4">
              {deleteTarget.name} ({deleteTarget.server_key})
            </p>
            <div className="flex justify-end gap-2">
              <button onClick={() => mcp.setDeleteConfirmId(null)} className="px-3 py-1.5 text-sm text-slate-400">
                {t({ ko: "취소", en: "Cancel" })}
              </button>
              <button
