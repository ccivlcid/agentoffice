...
export interface McpEditInitialData {
  name?: string;
  server_key?: string;
  package?: string;
  command?: string;
...
interface McpServerEditModalProps {
  ...
  initialData?: McpEditInitialData;
  agents: Agent[];
  submitting: boolean;
  t: TFunction;
  onClose: () => void;
...
export default function McpServerEditModal({
  ...
  const d = initialData;
  const [name, setName] = useState(server?.name ?? d?.name ?? "");
  const [serverKey, setServerKey] = useState(server?.server_key ?? d?.server_key ?? "");
  const [pkg, setPkg] = useState(server?.package ?? d?.package ?? "");
  const [command, setCommand] = useState(server?.command ?? d?.command ?? "npx");
  const [args, setArgs] = useState<string[]>(() => {
    try {
      return server?.args ? JSON.parse(server.args) : (d?.args ?? []);
    } catch {
      return d?.args ?? [];
    }
  });
  const [envEntries, setEnvEntries] = useState<Array<{ key: string; value: string }>>(() => {
    try {
      const obj = server?.env ? JSON.parse(server.env) : (d?.env ?? {});
      return Object.entries(obj).map(([key, value]) => ({ key, value: String(value) }));
    } catch {
      return [];
    }
  });
  const [description, setDescription] = useState(server?.description ?? d?.description ?? "");
  const [category, setCategory] = useState(server?.category ?? d?.category ?? "other");
  const [providers, setProviders] = useState<string[]>(() => {
    try {
      return server?.providers ? JSON.parse(server.providers) : (d?.providers ?? ["claude", "codex"]);
    } catch {
      return d?.providers ?? ["claude", "codex"];
    }
  });
      ...
      <div className="w-[calc(100vw-2rem)] max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl border border-slate-700 bg-slate-800 shadow-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-white">
            {isEdit
              ? t({ ko: "MCP 서버 편집", en: "Edit MCP Server" })
        ...
        <div className="space-y-3">
          ...
          <McpProviderGrid agents={agents} providers={providers} t={t} onToggle={toggleProvider} />
        </div>
        ...
        <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-slate-700/50">
          ...
          <button
            ...
            disabled={submitting || !name.trim() || !serverKey.trim()}
