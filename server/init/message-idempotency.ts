...
export function resolveMessageIdempotencyKey(
...
function findMessageByIdempotencyKey(db: Database, idempotencyKey: string): StoredMessage | null {
...
export async function withSqliteBusyRetry<T>(operation: string, fn: () => T): Promise<T> {
  for (let attempt = 0; attempt <= SQLITE_BUSY_RETRY_MAX_ATTEMPTS; attempt++) {
    try {
      const result = fn();
      if (attempt > 0) {
        console.warn(`[HyperClaw] SQLite busy recovered: op=${operation}, retries=${attempt}`);
      }
      return result;
    } catch (err) {
      ...
      console.warn(
        `[HyperClaw] SQLite busy: op=${operation}, attempt=${attempt + 1}/${SQLITE_BUSY_RETRY_MAX_ATTEMPTS + 1}, `
        + `retry_in=${waitMs}ms`,
      );
...
function insertMessageWithIdempotencyOnce(
  db: Database,
  input: MessageInsertInput,
): { message: StoredMessage; created: boolean } {
...
export function makeInsertMessageWithIdempotency(db: Database) {
  return async function insertMessageWithIdempotency(
    input: MessageInsertInput,
  ): Promise<{ message: StoredMessage; created: boolean }> {
    return withSqliteBusyRetry("messages.insert", () => insertMessageWithIdempotencyOnce(db, input));
  };
}

export function makeRollbackMessageInsertAfterAuditFailure(db: Database) {
  return async function rollbackMessageInsertAfterAuditFailure(messageId: string): Promise<void> {
...
export function makeRecordAcceptedIngressAuditOrRollback(
  recordMessageIngressAuditOr503: Function,
  rollbackMessageInsertAfterAuditFailure: Function,
) {
  return async function recordAcceptedIngressAuditOrRollback(
    res: { status(code: number): { json(payload: unknown): unknown } },
    input: Omit<any, "messageId">,
    messageId: string,
  ): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `[HyperClaw] rollback after audit failure failed: message_id=${messageId}, `
        + `${String(rollbackErr)}`,
      );
    }
    return false;
