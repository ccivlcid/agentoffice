...
export function initializeDatabaseRuntime(): {
  ...
  if (!process.env.DB_PATH && !fs.existsSync(DEFAULT_DB_PATH) && fs.existsSync(LEGACY_DB_PATH)) {
    ...
    console.log("[HyperClaw] Migrated database: climpire.sqlite -> hyperclaw.sqlite");
  }
  ...
  console.log(
    `[HyperClaw] SQLite write resilience: busy_timeout=${SQLITE_BUSY_TIMEOUT_MS}ms, `
  ...
  console.log(
    `[HyperClaw] Review guardrails: max_rounds=${REVIEW_MAX_ROUNDS}, `
  ...
  console.log(
    `[HyperClaw] In-progress watchdog: grace=${IN_PROGRESS_ORPHAN_GRACE_MS}ms, `
    + `sweep=${IN_PROGRESS_ORPHAN_SWEEP_MS}ms`,
  );
  console.log(
    `[HyperClaw] Subtask delegation sweep: interval=${SUBTASK_DELEGATION_SWEEP_MS}ms`,
  );
