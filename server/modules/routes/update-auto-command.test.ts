...
describe("parseSafeRestartCommand", () => {
  it("parses safe commands", () => {
    expect(parseSafeRestartCommand("pm2 restart hyperclaw")).toEqual({
      cmd: "pm2",
      args: ["restart", "hyperclaw"],
    });
    expect(parseSafeRestartCommand("\"/usr/local/bin/restart-helper\" gateway restart")).toEqual({
      cmd: "/usr/local/bin/restart-helper",
      args: ["gateway", "restart"],
    });
    expect(parseSafeRestartCommand("restart-helper \"some\\\"arg\" plain")).toEqual({
      cmd: "restart-helper",
      args: ["some\"arg", "plain"],
    });
  });
