...
// ---------------------------------------------------------------------------
// Builds the prompt used when delegating a subtask batch to another department
// ---------------------------------------------------------------------------

export function makeSubtaskDelegationPromptBuilder(deps: {
  db: any;
  ...
  return function buildSubtaskDelegationPrompt(
    ...
    const deptOwnedLabel = pickL(l(
      [`[${targetDeptName} 담당 업무 묶음]`], [`[${targetDeptName} owned batch]`],
      [`[${targetDeptName}担当タスク一式]`], [`[${targetDeptName}负责项集合]`],
    ), lang);
    const checklistLabel = pickL(l(
      ["순차 실행 체크리스트"], ["Sequential execution checklist"],
      ["順次実行チェックリスト"], ["顺序执行清单"],
    ), lang);
    ...
    return buildTaskExecutionPrompt([
      ...
      ``, `[${allSubtasksLabel}]`, subtaskLines, ``,
      deptOwnedLabel, `${checklistLabel}:`, orderedChecklist,
      conversationCtx ? `\n${conversationCtx}` : "", ``, `---`,
      ...
      deptConstraint, ``, finalInstruction,
    ], {
