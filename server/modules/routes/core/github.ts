// @ts-nocheck
/**
 * Core API: GitHub status, repos, branches, clone, project branches.
 */
...
export function registerCoreGitHub(ctx: RuntimeContext): void {
  const { app, db, broadcast } = ctx;
  function getGitHubAccessToken(): string | null {
    const row = db.prepare("SELECT access_token_enc, scope FROM oauth_accounts WHERE provider = 'github' AND status = 'active' ORDER BY priority ASC, updated_at DESC LIMIT 1")
  ...
  app.get("/api/github/status", async (_req, res) => {
    const row = db.prepare("SELECT id, email, scope, status, access_token_enc FROM oauth_accounts WHERE provider = 'github' AND status = 'active' ORDER BY priority ASC, updated_at DESC LIMIT 1")
  ...
  app.get("/api/github/repos", async (req, res) => {
    ...
    try {
      const url = q
        ? `https://api.github.com/search/repositories?q=${encodeURIComponent(q)}+user:@me&per_page=${perPage}&page=${page}&sort=updated`
  ...
  app.get("/api/github/repos/:owner/:repo/branches", async (req, res) => {
    const pat =
      typeof req.headers["x-github-pat"] === "string" ? req.headers["x-github-pat"].trim() : null;
  ...
  app.get("/api/projects/:id/branches", (req, res) => {
    const project = db
      ...
      .get(req.params.id) as { id: string; project_path: string } | undefined;
