// @ts-nocheck
/**
 * Project path helpers: allowed roots, normalization, browse, native picker.
 * Used by core/projects.ts. DB used only for findConflictingProjectByPath.
 */
...
export const PROJECT_PATH_SCOPE_CASE_INSENSITIVE =
  process.platform === "win32" || process.platform === "darwin";

/** Normalize path string (~, ~/, /Projects, etc.) to absolute path. */
export function normalizeProjectPathFromString(value: string): string | null {
...
export function parseProjectPathAllowedRootsEnv(raw: string | undefined): string[] {
  ...
  for (const part of parts) {
    const normalized = normalizeProjectPathFromString(part);
...
export const PROJECT_PATH_ALLOWED_ROOTS = parseProjectPathAllowedRootsEnv(
  process.env.PROJECT_PATH_ALLOWED_ROOTS
);

export function pathInsideRoot(candidatePath: string, rootPath: string): boolean {
  const rel = path.relative(rootPath, candidatePath);
  if (!rel) return true;
  return !rel.startsWith("..") && !path.isAbsolute(rel);
}

export function isPathInsideAllowedRoots(candidatePath: string): boolean {
...
export function getContainingAllowedRoot(candidatePath: string): string | null {
  if (PROJECT_PATH_ALLOWED_ROOTS.length === 0) return null;
  const normalizedCandidate = path.normalize(path.resolve(candidatePath));
  const containingRoots = PROJECT_PATH_ALLOWED_ROOTS.filter((root) =>
    pathInsideRoot(normalizedCandidate, root)
  );
...
export function pickDefaultBrowseRoot(): string | null {
...
export function findConflictingProjectByPath(
...
export function inspectDirectoryPath(targetPath: string): {
  exists: boolean;
  isDirectory: boolean;
  canCreate: boolean;
...
export function ensureDirectoryPathExists(
  targetPath: string
...
export function collectProjectPathSuggestions(query: string, limit: number): string[] {
  const roots =
...
export function findNearestExistingDirectory(targetPath: string): string | null {
...
export function resolveInitialBrowsePath(
...
// execFileText and pickNativeDirectoryPath moved to project-path-picker.ts
export { execFileText, pickNativeDirectoryPath } from "./project-path-picker.ts";
