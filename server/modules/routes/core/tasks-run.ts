// @ts-nocheck
/** Core API: task run endpoint (POST /api/tasks/:id/run). */

import type { RuntimeContext } from "../../../types/runtime-context.ts";
import path from "node:path";

export function registerTaskRun(ctx: RuntimeContext): void {
  const {
    app,
    db,
    broadcast,
  ...
  `).get(agentId) as {
    ...
    const executionMode = req.body?.execution_mode as string | undefined;
    let worktreePath: string | null = null;
    if (executionMode !== "direct") {
      worktreePath = createWorktree(projectPath, id, agent.name);
    }
...
\`\`\``],
    ...
    const mainReasoningLevel = agent.cli_reasoning_level || modelConfig[provider]?.reasoningLevel || undefined;
    ...
    const prompt = buildTaskExecutionPrompt([
      ...
      agent.personality ? `Personality: ${agent.personality}. Act in character: your work and replies should reflect this.` : "",
      deptConstraint,
    ...
    const markTaskStarted = (agentId: string, taskId: string) => {
      const t = nowMs();
      db.prepare(
        "UPDATE tasks SET status = 'in_progress', assigned_agent_id = ?, started_at = ?, updated_at = ? WHERE id = ?"
      ).run(agentId, t, t, taskId);
      db.prepare("UPDATE agents SET status = 'working', current_task_id = ? WHERE id = ?").run(taskId, agentId);
      ...
      const taskRow = db.prepare("SELECT department_id FROM tasks WHERE id = ?").get(taskId) as { department_id: string | null } | undefined;
      startProgressTimer(taskId, task.title, taskRow?.department_id ?? null);
    };
    ...
    // CLI agent (claude/codex/gemini/opencode)
