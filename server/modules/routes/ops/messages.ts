...
import { buildDecisionStateHelpers } from "./messages-decision-state.ts";
import { buildPlanningConsolidationHelpers } from "./messages-planning-consolidation.ts";
import { buildInboxItemHelpers } from "./messages-inbox-items.ts";
...
export function registerOpsMessageRoutes(ctx: RuntimeContext): any {
  ...
  const scheduleTeamLeaderReplies = __ctx.scheduleTeamLeaderReplies;
  ...
  const buildAgentUpgradeRequiredPayload = () => {
    ...
    const agentsTemplate = path.join(repoRoot, "templates", "AGENTS-empire.md");
    const installCommands = {
      mac_linux: "pnpm setup",
      windows_powershell: "pnpm setup",
      agents_only: "pnpm setup -- --agents-path <path-to-AGENTS.md>",
    };
    return {
      error: "agent_upgrade_required",
      reason: "install_first",
      message: `AGENTS rules are outdated (HTTP 428). Run: pnpm setup. Script: ${setupScript}.`,
      message_ko: `AGENTS 규칙이 구버전입니다(HTTP 428). pnpm setup 실행. 스크립트: ${setupScript}.`,
      required_agent_rules_version: 2,
      required_action: "install_latest_agents_rules",
      installer_files: ["scripts/setup.mjs", "templates/AGENTS-empire.md"],
      installer_absolute_paths: { setup_mjs: setupScript, agents_template: agentsTemplate },
      install_commands: installCommands,
      user_prompt: {
        ko: "AGENTS 규칙이 구버전입니다. pnpm setup을 실행한 뒤 같은 directive를 1회 재시도하세요.",
        en: "AGENTS rules are outdated. Run pnpm setup and retry the same directive once.",
      },
      next_step_ko: "사용자가 동의하면 pnpm setup 실행 후 같은 directive를 1회 재시도하세요.",
      next_step_en: "If the user agrees, run pnpm setup and retry the same directive once.",
    };
  };
