...
export function buildReviewRoundConsolidation(ctx: {
  db: any;
  nowMs: () => number;
  ...
  const { db, nowMs, findTeamLeader, getAgentDisplayName, runAgentOneShot, chooseSafeReply, l, pickL, stateHelpers, reviewRoundDecisionConsolidationInFlight } = ctx;
  const { getProjectReviewDecisionState, getReviewRoundDecisionState, upsertReviewRoundDecisionState, recordProjectReviewDecisionEvent, formatPlannerSummaryForDisplay } = stateHelpers;

  function buildReviewRoundPlanningFallbackSummary(
    lang: string, taskTitle: string, reviewRound: number, optionNotes: string[], projectName?: string | null,
  ): string {
    ...
    const lines = optionNotes.slice(0, 6).map((note, idx) => `${idx + 1}. ${clip(note)}`).filter(Boolean);
    const optionBlock = lines.length > 0
      ? lines.join("\n")
      : pickL(l(["- 취합할 라운드 의견이 없습니다."], ["- No round opinions to consolidate."], ["- 集約対象のラウンド意見がありません。"], ["- 暂无可汇总的轮次意见。"]), lang);
    return pickL(l(
  ...
  function queueReviewRoundPlanningConsolidation(input: {
    projectId: string | null; projectName: string | null; projectPath: string | null;
    taskId: string; taskTitle: string; meetingId: string; reviewRound: number;
    optionNotes: string[]; snapshotHash: string; lang: string;
  }): void {
    ...
    void (async () => {
      try {
        ...
        const fallbackSummary = buildReviewRoundPlanningFallbackSummary(input.lang, input.taskTitle, input.reviewRound, input.optionNotes, input.projectName);
        ...
        if (planningLeader) {
          const sourceBlock = input.optionNotes.length > 0
            ...
            : pickL(l(["- 라운드 의견 없음"], ["- No round opinions"], ["- ラウンド意見なし"], ["- 无轮次意见"]), input.lang);
          const prompt = [
            ...
            "", "Round option sources:", sourceBlock,
          ].join("\n");
          try {
            const run = await runAgentOneShot(planningLeader, prompt, { projectPath: input.projectPath || process.cwd(), timeoutMs: REVIEW_MEETING_ONESHOT_TIMEOUT_MS, noTools: true });
        ...
        db.prepare(`UPDATE review_round_decision_states SET status = 'failed', planner_summary = ?, updated_at = ? WHERE meeting_id = ? AND snapshot_hash = ?`).run(failMsg, nowMs(), input.meetingId, input.snapshotHash);
      } finally {
  ...
  return { buildReviewRoundPlanningFallbackSummary, queueReviewRoundPlanningConsolidation };
}
