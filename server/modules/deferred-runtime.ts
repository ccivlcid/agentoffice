...
export function assertNoUnresolvedDeferredRuntimeFunctions(
  ...
  if (unresolved.length > 0) {
    throw new Error(`[HyperClaw] ${label} incomplete: ${unresolved.join(", ")}`);
  }
}
...
export function assertRuntimeFunctionsPresent(
  ...
  const missing = [...new Set(functionNames)]
    ...
    .sort();
  if (missing.length > 0) {
    throw new Error(`[HyperClaw] ${label} missing functions: ${missing.join(", ")}`);
  }
}
...
export function assertRuntimeFunctionsResolved(
  ...
  if (missing.length > 0 || unresolved.length > 0) {
    ...
    throw new Error(`[HyperClaw] ${label} ${parts.join(" | ")}`);
  }
}
