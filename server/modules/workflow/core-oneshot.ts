...
import { normalizeConversationReply } from "./core-meeting-utils.ts";
...
export function createSafeLogStreamOps(logStream: any): {
...
export async function runAgentOneShot(
  ...
  const logPath = path.join(deps.logsDir, `${runId}.log`);
  ...
  const detachChildListeners = () => {
    ...
    if (!child) return;
    if (activeStdoutListener) { child.stdout?.off("data", activeStdoutListener); activeStdoutListener = null; }
    if (activeStderrListener) { child.stderr?.off("data", activeStderrListener); activeStderrListener = null; }
    if (activeErrorListener) { child.off("error", activeErrorListener); activeErrorListener = null; }
    if (activeCloseListener) { child.off("close", activeCloseListener); activeCloseListener = null; }
    activeChild = null;
  };
  ...
  const onChunk = (chunk: Buffer | string, stream: "stdout" | "stderr") => {
    ...
    rawOutput += text;
    safeWrite(text);
    if (streamTaskId) deps.broadcast("cli_output", { task_id: streamTaskId, stream, data: text });
  };
  ...
  try {
    if (provider === "api") {
      ...
      try {
        await deps.executeApiProviderAgent(
          prompt, projectPath, logStream, controller.signal, streamTaskId ?? undefined,
          (agent as any).api_provider_id ?? null, (agent as any).api_model ?? null,
          (text: string) => { rawOutput += text; return safeWrite(text); },
        );
      ...
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), timeoutMs);
      const oauthWrite = (text: string) => { rawOutput += text; return safeWrite(text); };
      try {
        if (provider === "copilot") {
          await deps.executeCopilotAgent(prompt, projectPath, logStream, controller.signal, streamTaskId ?? undefined, agent.oauth_account_id ?? null, oauthWrite);
        } else {
          await deps.executeAntigravityAgent(prompt, logStream, controller.signal, streamTaskId ?? undefined, agent.oauth_account_id ?? null, oauthWrite);
        }
      } finally { clearTimeout(timeout); }
      if (!rawOutput.trim() && fs.existsSync(logPath)) rawOutput = fs.readFileSync(logPath, "utf8");
    } else {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const child = spawn(args[0], args.slice(1), {
          cwd: projectPath, env: cleanEnv, shell: process.platform === "win32",
          stdio: ["pipe", "pipe", "pipe"], detached: false, windowsHide: true,
        });
        activeChild = child;
        let settled = false;
        const settle = (cb: () => void) => { if (settled) return; settled = true; detachChildListeners(); cb(); };

        const timeout = setTimeout(() => {
          const pid = child.pid ?? 0;
          detachChildListeners();
          if (pid > 0) deps.killPidTree(pid);
          settle(() => reject(new Error(`timeout after ${timeoutMs}ms`)));
        }, timeoutMs);

        activeErrorListener = (err: Error) => { clearTimeout(timeout); settle(() => reject(err)); };
        activeStdoutListener = (chunk: Buffer) => onChunk(chunk, "stdout");
        activeStderrListener = (chunk: Buffer) => onChunk(chunk, "stderr");
        activeCloseListener = (code: number | null) => { clearTimeout(timeout); exitCode = code ?? 1; settle(() => resolve()); };
    ...
    if (opts.rawOutput) {
      ...
      const pretty = deps.prettyStreamJson(rawOutput).trim();
      if (pretty) return { text: pretty, error: message };
      return { text: "", error: message };
    }
    const partial = normalizeConversationReply(rawOutput, deps.prettyStreamJson, 320);
    if (partial) return { text: partial, error: message };
    const pretty = deps.prettyStreamJson(rawOutput);
  ...
  if (exitCode !== 0 && !rawOutput.trim()) return { text: "", error: `${provider} exited with code ${exitCode}` };

  if (opts.rawOutput) {
    const pretty = deps.prettyStreamJson(rawOutput).trim();
    return { text: pretty || rawOutput.trim() };
  }

  const normalized = normalizeConversationReply(rawOutput, deps.prettyStreamJson);
  ...
  const lang = deps.getPreferredLanguage();
