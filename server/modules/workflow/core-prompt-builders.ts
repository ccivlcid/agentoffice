...
export function buildDirectReplyPrompt(
  ...
  const prompt = [
    ...
    "- Judge: Is the CEO requesting WORK (create, make, fix, build, write, update, design, implement, etc.) or just CHATTING (greeting, question, opinion)?",
    '- WORK → Your reply MUST start with exactly "[TASK]" on the very first line. Then briefly confirm the task (1-2 sentences).',
    '  Example: "[TASK]\\nPPT를 만들겠습니다. 바이브 코딩 관점에서 10장 슬라이드로 구성하겠습니다."',
    "- CHAT → Reply naturally (1-3 sentences). Do NOT include [TASK].",
    '  Example: "안녕하세요! 무엇을 도와드릴까요?"',
    "- Return one direct response only. No JSON, no code blocks.",
