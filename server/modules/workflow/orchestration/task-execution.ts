...
export function createTaskExecutionHelpers(ctx: {
  db: any;
  nowMs: () => number;
  ...
  l: (...args: any[]) => any;
  logsDir: string;
  getDeptRoleConstraint: (...args: any[]) => string;
  getDeptName: (...args: any[]) => string;
  getAgentDisplayName: (...args: any[]) => string;
  getRecentConversationContext: (...args: any[]) => any;
  getTaskContinuationContext: (...args: any[]) => any;
  getRecentChanges: (...args: any[]) => string;
  getProviderModelConfig: () => any;
  getNextHttpAgentPid: () => number;
  resolveProjectPath: (...args: any[]) => string;
  createWorktree: (...args: any[]) => string | null;
  ensureClaudeMd: (...args: any[]) => void;
  buildAvailableSkillsPromptBlock: (...args: any[]) => string;
  buildTaskExecutionPrompt: (...args: any[]) => string;
  hasExplicitWarningFixRequest: (...args: any[]) => boolean;
  launchApiProviderAgent: (...args: any[]) => void;
  launchHttpAgent: (...args: any[]) => void;
  spawnCliAgent: (...args: any[]) => any;
  startProgressTimer: (taskId: string, taskTitle: string, deptId: string | null) => void;
  ...
  const {
    ...
    resolveLang, pickL, l, logsDir, getDeptRoleConstraint, getDeptName,
    getAgentDisplayName, getRecentConversationContext, getTaskContinuationContext,
    getRecentChanges, getProviderModelConfig, getNextHttpAgentPid,
    resolveProjectPath, createWorktree, ensureClaudeMd,
    buildAvailableSkillsPromptBlock, buildTaskExecutionPrompt, hasExplicitWarningFixRequest,
    launchApiProviderAgent, launchHttpAgent, spawnCliAgent,
    startProgressTimer, ensureTaskExecutionSession, handleTaskRunComplete, randomDelay,
  } = ctx;

  function startProgressTimerLocal(taskId: string, taskTitle: string, departmentId: string | null): void {
    startProgressTimer(taskId, taskTitle, departmentId);
  }
  ...
  function startTaskExecutionForAgent(
    ...
    startProgressTimerLocal(taskId, taskData.title, deptId);
  }

  return { startTaskExecutionForAgent };
}
