...
export async function runReviewVotePhase(
  params: any,
  ctx: any,
  onApproved: () => void,
): Promise<void> {
  const {
    taskId, taskTitle, round, roundMode,
    isRound1Remediation, isRound2Merge, isFinalDecisionRound,
    meetingId, leaders, planningLeader, transcript, lang,
  } = params;
  let { needsRevision, reviseOwner } = params;
  ...
  const {
    ...
    appendTaskLog, notifyCeo, scheduleNextReviewRound,
    l, pickL, getAgentDisplayName, getDeptName,
    finishMeetingMinutes, dismissLeadersFromCeoOffice,
    speak, abortIfInactive, oneShotOptions,
  ...
  const taskDescription = db.prepare("SELECT description FROM tasks WHERE id = ?")
    .get(taskId)?.description ?? null;
  ...
  for (const leader of leaders) {
    ...
    if (isDeferrableReviewHold(latestDecisionLine)) {
      const clipped = summarizeForMeetingBubble(latestDecisionLine, 160, lang as any);
      ...
      appendTaskLog(taskId, "system", `Review round ${round}: converted deferrable hold to post-merge monitoring (${leader.id})`);
      continue;
    }
    if (finalHoldLeaders.length >= REVIEW_MAX_REVISION_SIGNALS_PER_ROUND) {
      appendTaskLog(taskId, "system", `Review round ${round}: hold signal ignored (round cap ${REVIEW_MAX_REVISION_SIGNALS_PER_ROUND})`);
      continue;
    }
    ...
    if (deptCount >= REVIEW_MAX_REVISION_SIGNALS_PER_DEPT_PER_ROUND) {
      appendTaskLog(taskId, "system", `Review round ${round}: hold signal ignored for dept ${deptKey} (dept cap ${REVIEW_MAX_REVISION_SIGNALS_PER_DEPT_PER_ROUND})`);
      continue;
    }
  ...
  if (!needsRevision && deferredMonitoringNotes.length > 0) {
    ...
    appendTaskLog(taskId, "system", `Review round ${round}: deferred ${deferredMonitoringLeaders.length} hold opinions to SLA monitoring checklist`);
  }
  ...
  if (needsRevision) {
    const rawMemoItems = collectRevisionMemoItems(transcript, REVIEW_MAX_MEMO_ITEMS_PER_ROUND, REVIEW_MAX_MEMO_ITEMS_PER_DEPT);
    ...
    if ((isRound1Remediation || isRound2Merge) && !remediationLimitReached) {
      ...
      appendTaskLog(taskId, "system", `${REVIEW_DECISION_PENDING_LOG_PREFIX} (round=${round}, options=${memoItemsForAction.length})`);
      notifyCeo(pickL(l(
    ...
    if ((isRound1Remediation || isRound2Merge) && remediationLimitReached) {
      appendTaskLog(taskId, "system", `Review consensus round ${round}: remediation request cap reached (${REVIEW_MAX_REMEDIATION_REQUESTS}/task), skipping additional remediation`);
      notifyCeo(pickL(l(
    ...
    appendTaskLog(taskId, "system", `Review consensus round ${round}: forcing finalization with documented residual risk (${forceReason})`);
