...
export function createPlannedMeetingHelpers(ctx: {
  ...
  chooseSafeReply: (...args: any[]) => string;
  buildMeetingPrompt: (...args: any[]) => string;
  ...
  beginMeetingMinutes: (...args: any[]) => string;
  finishMeetingMinutes: (...args: any[]) => void;
  appendMeetingMinuteEntry: (...args: any[]) => void;
  callLeadersToCeoOffice: (...args: any[]) => void;
  dismissLeadersFromCeoOffice: (...args: any[]) => void;
  emitMeetingSpeech: (...args: any[]) => void;
  collectPlannedActionItems: (...args: any[]) => string[];
  appendTaskProjectMemo: (...args: any[]) => void;
}) {
  const {
    db, nowMs, appendTaskLog, broadcast, notifyCeo, resolveLang, pickL, l,
    randomDelay, sleepMs, getAgentDisplayName, getDeptName, getRoleLabel,
    resolveProjectPath, sendAgentMessage, runAgentOneShot, chooseSafeReply,
    buildMeetingPrompt, reviewRoundState, reviewInFlight,
    isTaskWorkflowInterrupted, getTaskStatusById, clearTaskWorkflowState,
    getTaskReviewLeaders, beginMeetingMinutes, finishMeetingMinutes,
    appendMeetingMinuteEntry, callLeadersToCeoOffice, dismissLeadersFromCeoOffice,
    emitMeetingSpeech, collectPlannedActionItems, appendTaskProjectMemo,
  } = ctx;
  ...
  return { startPlannedApprovalMeeting };
}
