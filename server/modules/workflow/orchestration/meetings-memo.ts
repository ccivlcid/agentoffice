...
export interface MemoCtx {
  db: any;
  nowMs: () => number;
  appendTaskLog: (taskId: string, level: string, msg: string) => void;
  broadcast: (event: string, data: any) => void;
  summarizeForMeetingBubble: (text: string, maxLen: number, lang?: any) => string;
  l: (...args: any[]) => any;
  pickL: (val: any, lang: any) => string;
}

export function makeMemoHelpers(ctx: MemoCtx) {
  const { db, nowMs, appendTaskLog, broadcast, summarizeForMeetingBubble, l, pickL } = ctx;
  ...
  function appendTaskReviewFinalMemo(
    ...
    for (let i = transcript.length - 1; i >= 0; i -= 1) {
      ...
      const clipped = summarizeForMeetingBubble(row.content, 140, lang as any);
      if (!clipped) continue;
