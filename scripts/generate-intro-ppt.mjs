/**
 * HyperClaw Introduction Presentation Generator
...
pptx.company = "HyperClaw";
pptx.subject = "HyperClaw Introduction";
pptx.title = "HyperClaw — AI Agent Office Simulator";
...
pptx.defineSlideMaster({
  ...
  objects: [
    ...
    { text: { text: "HyperClaw", options: { x: 0.4, y: "93%", w: 3, h: 0.4, fontSize: 9, color: C.textDim, fontFace: "Arial" } } },
...
// ═══════════════════════════════════════════════
{
  ...
  s.addText("github.com/YOUR_ORG/hyperclaw", {
    x: 0.8, y: 4.8, w: 8, h: 0.3,
    fontSize: 11, fontFace: "Arial", color: C.textDim,
    hyperlink: { url: "https://github.com/YOUR_ORG/hyperclaw" },
  });
}
...
// ═══════════════════════════════════════════════
{
  ...
  const cols = [
    ...
    },
    {
      title: "Integrations",
      color: C.yellow,
      items: ["6 CLI Providers", "8 External API Providers", "GitHub / Google OAuth", "AGENTS.md Injection", "자체 웹훅·메신저 연동 구현 가능"],
    },
  ];
...
// ═══════════════════════════════════════════════
{
  ...
  const features = [
    ...
    { icon: "💬", title: "CEO Chat & Directives", desc: "$-prefix 지시 시스템. 팀 리더 직접 대화. 전사 공지. 메신저 연동은 자체 웹훅 구현.", color: C.yellow },
...
{
  const s = pptx.addSlide({ masterName: "MASTER" });
  addTitle(s, "외부 연동 (자체 구현)");

  s.addText("웹 UI·REST API로 CEO 지시 및 태스크 관리. 메신저(Telegram/Discord/Slack) 연동은 자체 웹훅/인박스 구현으로 확장 가능합니다.", {
  ...
  s.addText("자체 웹훅 엔드포인트 구현 후 메신저 Bot 연동. (기존 /api/inbox 제거됨)", {
...
// ═══════════════════════════════════════════════
{
  ...
  const links = [
    ["GitHub", "github.com/YOUR_ORG/hyperclaw"],
    ["License", "Apache 2.0"],
