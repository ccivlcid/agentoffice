...
전체 디렉터리/파일 구조는 [source-tree.txt](source-tree.txt)를 참고하세요.

```text
haiferclaw/
├── server/          # Express 5 + SQLite + WebSocket (config, db, gateway, oauth, security, ws, modules/, types/)
├── src/             # React 19 + Vite (components/, hooks/, types/, api, App, main)
├── public/          # sprites/, 정적 자산
├── scripts/         # setup.mjs, auto-apply, generate-*, preflight, qa/
├── docs/            # architecture/, releases/, reports/, DESIGN.md 등
├── templates/       # AGENTS-empire.md
├── tests/e2e/       # Playwright
├── index.html, vite.config.ts, tsconfig*.json, vitest.config.ts, playwright.config.ts
