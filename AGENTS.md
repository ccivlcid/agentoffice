...
- KO: `✅ HyperClaw 업무지시 전달 완료` (회의 진행) / `✅ HyperClaw 업무지시 전달 완료 (회의 생략)` (회의 없이)
...
The HyperClaw server detects the `$` prefix and automatically:
...
1. **`$` directive**: After collecting required meeting/path inputs and sending to API, reply with only `✅ HyperClaw 업무지시 전달 완료` (or language equivalent). Nothing else.
